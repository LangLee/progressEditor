<template>
  <div
    class="h-screen flex flex-col items-center justify-center lg:bg-slate-300/20 p-2 lg:p-4"
  >
    <div
      class="w-full lg:w-96 flex flex-col bg-white p-8 rounded-md lg:shadow-md"
    >
      <div
      ...
      <div class="w-full my-2 flex flex-row justify-between">
        ...
        />
        <button
          class="px-2 bg-slate-300 text-white font-semibold rounded-md hover:bg-slate-600"
          @click.stop="onSign"
        >
          {{ verifying ? '重新发送' : '发送验证码' }}
        </button>
      </div>
      <ProInput
        class="my-2"
...
const onChangePassword = () => {
  ...
  verifyAndUpdatePassword(
    { email: email.value, password: md5(password.value) },
    verification.value
  ).then((result) => {
    if (result) {
      router.push('login')
