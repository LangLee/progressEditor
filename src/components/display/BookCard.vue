<template>
    <div class="flex flex-row w-full h-28 rounded-lg shadow-lg mb-2 lg:mb-4 p-2 bg-white dark:bg-neutral-800">
        ...
        <div class="flex-1 flex flex-col">
            <div class="text-xl font-semibold text-slate-700 mb-2 dark:text-gray-200">{{ book.title }}</div>
            <div class="flex flex-row text-sm text-gray-400 dark:text-gray-500 mb-2">
                <!-- <span>{{ book.author }}</span> -->
                <span>{{ book.updateTime }}</span>
            </div>
            <div class="text-gray-500 dark:text-gray-400">{{ book.description || '这是描述信息...' }}</div>
        </div>
    </div>
</template>
<script setup>
