<template>
  <div
    ...
    :class="foldState ? 'translate-x-[-100%]' : 'translate-x-0'"
    @click.stop="doFold"
  >
    <div
      v-if="editable && groups && groups.length > 0"
      class="flex lg:mx-4 py-2 bg-white/95 lg:bg-transparent dark:bg-neutral-900/60 font-sans-serif font-medium"
    >
      <button
        class="h-8 leading-8 text-blue-500 hover:text-blue-700 dark:text-blue-300 dark:hover:text-blue-500 mx-2"
    ...
    <nav
      ...
      class="relative flex-1 px-1 sm:px-3 xl:px-5 pb-4 bg-white/95 lg:bg-transparent dark:bg-neutral-900/60 overflow-y-auto overflow-x-hidden"
    >
      <ul>
        <li v-for="(group, index) in groups" :key="group.id">
          <input
            ref="titleInput"
            v-if="group.id === editItem"
            class="p-2 w-full rounded-md"
            type="text"
            v-model="group.name"
            @keyup.enter="onUpdateGroup(group)"
          />
          <div
            v-else
            class="px-1 h-10 leading-10 relative font-medium text-gray-700 dark:text-gray-300 cursor-pointer"
            @dblclick.stop="onInputEditGroup(group.id)"
            @mouseover.stop="onItemMouseover(group.id)"
          ...
          <ul class="ml-2 px-1 border-l border-gray-200 dark:border-gray-500">
            ...
            >
              <input
                ref="titleInput"
                v-if="book.id === editItem"
                class="p-2 w-full rounded-md"
                type="text"
                v-model="book.title"
                @keyup.enter="onUpdateBook(book)"
              />
              <a
                v-else
                @click.stop="onMenuChange(book)"
                @dblclick.stop="onEditBookTitle(book.id)"
                @mouseover="onItemMouseover(book.id)"
              ...
              >
                ...
                >
                  <RemixIcon
                    class="text-blue-300 mr-1"
                    :name="getIconByType(book.type)"
                  ></RemixIcon>
                  <span class="break-all">{{ book.title }}</span>
                </span>
                <transition name="slide">
                  <div
                    v-if="editable && hoverItem === book.id"
                    class="absolute right-2 top-1 font-sans text-slate-50"
                  >
                    <span
                      class="rounded cursor-pointer bg-slate-300 hover:bg-slate-500 mr-1"
                      @click.stop="onShareBook(book)"
                    >
                      <RemixIcon
                        :name="book.share ? 'eye-off-line' : 'share-line'"
                      />
                      <!-- <span>分享</span> -->
                    </span>
                    <span
                      class="rounded cursor-pointer bg-blue-300 hover:bg-blue-500 mr-1"
                    ...
                    <span
                      class="rounded cursor-pointer bg-red-300 hover:bg-red-500"
    ...
    <BookModal
      :fixedType="fixedType"
      :title="`${isNew ? '新增' : '编辑'}书籍`"
      :visible="!!editBook"
      @confirm="finishEditBook"
      @cancel="closeModal"
      :book="editBook"
      :categories="groups"
    ></BookModal>
